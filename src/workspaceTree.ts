...
export function registerWorkspaceTree(context: vscode.ExtensionContext) {
	...
	vscode.commands.registerCommand(
		...
		async (item: StorageTreeItem) => {
			...
			if (!item) {
				newDirUri = vscode.Uri.parse(
					path.posix.join(
						workspaceTreeProvider.storage.path ?? '',
						name
					)
				);
			} else {
	...
	vscode.commands.registerCommand('workspace.refreshEntry', () =>
...
class StorageTreeProvider implements vscode.TreeDataProvider<StorageTreeItem> {
	...
	private _onDidChangeTreeData: vscode.EventEmitter<
		StorageTreeItem | undefined | void
	...
	async getChildren(
		element?: StorageTreeItem
	): Promise<StorageTreeItem[] | null | undefined> {
		if (!element) {
	...
	getParent(
		element: StorageTreeItem
	): vscode.ProviderResult<StorageTreeItem> {
	...
	async _getItems(
	...
	): Promise<vscode.ProviderResult<StorageTreeItem[]>> {
		if (!uri) {
			return undefined;
		}
		...
		for (const file of files) {
			...
			const itemPath = vscode.Uri.parse(
				path.posix.join(uri.path, file[0])
			);
...
export class StorageTreeItem extends vscode.TreeItem {
	...
	constructor(uri: vscode.Uri, name: string, fileType: vscode.FileType) {
